...
class VRNN_ss(nn.Module):
    ...
    def generation_x(self, feature_zt, h_t):
        ...
        y_t = self.gen_logvar(dec_output)
    ...
    def recurrence(self, feature_xt, feature_zt, h_t, c_t):
        ...
        rnn_input = torch.cat((feature_xt, feature_zt), -1).unsqueeze(0)
    ...
    def forward(self, x, use_pred=1):
        ...
        for t in range(seq_len):
            ...
            if random_num <= use_pred:
                feature_xt_pred = torch.exp(y_t)
                feature_xt_pred = self.feature_extractor_x(feature_xt_pred)
                h_t, c_t = self.recurrence(feature_xt_pred, feature_zt, h_t, c_t) # recurrence for t+1, with prediction
            else:
                h_t, c_t = self.recurrence(feature_xt, feature_zt, h_t, c_t) # recurrence for t+1, with GT
