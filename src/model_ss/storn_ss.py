...
storn_ss.py add the schedule sampling to original STORN
"""
...
def build_STORN_ss(cfg, device='cpu'):
    ...
    # Build model
    model = STORN_ss(x_dim=x_dim, z_dim=z_dim, activation=activation,
...
class STORN_ss(nn.Module):
    ...
    def generation_x(self, z, x_tm1, use_pred=1):
        ...
        x_tmp= torch.zeros(1, batch_size, self.x_dim).to(self.device)

        # 1. extract feature if needed
        z_h = self.mlp_z_h(z)

        # 2. recurrently generate y
        for t in range(seq_len):
            random_num = np.random.uniform()
            if random_num <= use_pred:
                xtm1_h = self.mlp_xtm1_h(x_tmp) # use previous prediction
            else:
                xtm1_h = self.mlp_xtm1_h(x_tm1[t,:,:].unsqueeze(0)) # use ground-truth
            

            zx_h = torch.cat((z_h[t,:,:].unsqueeze(0), xtm1_h), -1)
            y_t, (h_t, c_t) = self.rnn_h(zx_h, (h_t, c_t))
            
            y_t = self.mlp_h_x(y_t)
            y_t = self.gen_logvar(y_t)
            y[t,:,:] = y_t[0] # (1, bs, h_out) -> (bs, h_out)

            x_tmp = torch.exp(y_t.detach())
    ...
    def forward(self, x, use_pred=1):
        ...
        y = self.generation_x(self.z, x_tm1, use_pred)
