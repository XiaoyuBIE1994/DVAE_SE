...
from .model import build_VAE, build_DKF, build_SRNN, build_RVAE
...
class LearningAlgorithm():
    ...
    def train(self):
        ...
        kl_warm_step = self.cfg.getint('Training', 'kl_warm_step')
        ...
        for epoch in range(start_epoch+1, epochs):
            ...
            if epoch < kl_warm_step:
                kl_warm = epoch / kl_warm_step
            else:
                kl_warm = 1

            if epoch == 0:
                logger.info('=====> KL warm up start')
            elif epoch == kl_warm_step:
                logger.info('=====> KL warm up end')
            ...
            # Early stop patiance
            if val_loss[epoch] < best_val_loss or epoch < kl_warm_step:
