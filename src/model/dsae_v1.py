...
class DSAE(nn.Module):
    ...
    def build(self):
        ...
        self.mlp_zh_h = nn.Sequential(nn.Linear(self.dim_RNN_prior+self.z_dim, self.dim_RNN_prior), nn.Tanh())
        self.mlp_zc_c = nn.Sequential(nn.Linear(self.dim_RNN_prior+self.z_dim, self.dim_RNN_prior), nn.Tanh())
        self.mlp_z_z =nn.Sequential(nn.Linear(self.z_dim, self.z_dim), nn.Tanh())
    ...
    def generation_z(self, z_tm1):
        ...
        seq_len, batch_size, dim_z  = z_tm1.shape
        assert self.num_RNN_prior == 1 # only support one LSTM for instance
        z_mean_p = torch.zeros((seq_len, batch_size, self.z_dim)).to(self.device)
        z_logvar_p = torch.zeros((seq_len, batch_size, self.z_dim)).to(self.device)
        h_t = torch.zeros(1, batch_size, self.dim_RNN_prior).to(self.device)
        c_t = torch.zeros(1, batch_size, self.dim_RNN_prior).to(self.device)
        for t in range(seq_len):
            z_tmp = z_tm1[t].unsqueeze(0) # (1, bs, dim_z)
            h_t = self.mlp_zh_h(torch.cat((h_t, z_tmp),-1))
            c_t = self.mlp_zh_h(torch.cat((c_t, z_tmp),-1))
            z_tmp = self.mlp_z_z(z_tmp)
            z_p, (h_t, c_t) = self.rnn_prior(z_tmp, (h_t, c_t))
            z_mean_p[t,:,:] = self.prior_mean(z_p).squeeze()
            z_logvar_p[t,:,:] = self.prior_logvar(z_p).squeeze()
    ...
    def generation_x(self, v, z):
        ...
        # mlp to output y
        y = self.gen_logvar(self.mlp_vz_x(vz_cat))
